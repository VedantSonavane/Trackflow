...
import { RefreshCw, TrendingDown, Plus, X } from 'lucide-react';
...
const MOCK_FUNNELS = [
  ...
  },
  {
    ...
    steps: [
      { label: 'Product page', count: 1800 },
      { label: 'Add to cart', count: 940 },
      { label: 'Checkout', count: 520 },
      { label: 'Payment', count: 310 },
      { label: 'Thank you', count: 290 },
    ],
  },
];
...
export default function SiteFunnels() {
  ...
  useEffect(() => { fetchFunnels(customSteps); }, [id, range]);
    ...
    <div className="p-6 flex-1 overflow-auto bg-trackflow-bg">
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => fetchFunnels(customSteps)}
      ...
      {showCustom && (
        <div className="bg-white border border-trackflow-bg-3 rounded-xl p-4 mb-5">
          <p className="text-[11px] text-trackflow-text-3 mb-3">Enter page paths to define a custom funnel (e.g. <code className="font-mono bg-trackflow-bg-2 px-1 rounded">/pricing</code>)</p>
          <div className="flex gap-2 mb-3">
            ...
            <button
              onClick={addStep}
              className="px-3 py-1.5 bg-trackflow-accent text-white rounded-md text-xs font-sans cursor-pointer hover:bg-trackflow-accent-hover"
            >
              Add
            </button>
          </div>
            ...
            <div className="flex gap-1 flex-wrap">
              {customSteps.map((s, i) => (
                <div key={s} className="flex items-center gap-1 bg-trackflow-bg-2 rounded px-2 py-0.5">
                  ...
                  <button onClick={() => removeStep(s)} className="text-trackflow-text-3 hover:text-red-500 ml-1">
                    <X size={10} />
                  </button>
                </div>
              ))}
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-5">
          {funnels.map((funnel, fi) => {
            ...
            return (
              <div key={fi} className="bg-white border border-trackflow-bg-3 rounded-xl overflow-hidden">
                ...
                <div className="p-6 flex flex-col gap-0">
                  {funnel.steps.map((step, si) => {
                    ...
                    const dropOff = si > 0
                      ? Math.round(((funnel.steps[si - 1].count - step.count) / funnel.steps[si - 1].count) * 100)
                      : null;
                    const convRate = Math.round((step.count / maxCount) * 100);
                    return (
                      <div key={si}>
                        <div className="flex items-center gap-4 py-3">
                          ...
                          <div className="flex-1 h-7 bg-trackflow-bg-2 rounded-md overflow-hidden">
                            <div
                              className="h-full bg-trackflow-text rounded-md transition-all duration-700"
                          ...
                          <div className="w-28 shrink-0 flex items-center justify-between gap-2">
                            ...
                            <span className="text-[10px] text-trackflow-text-3">{convRate}%</span>
                          </div>
                          {dropOff !== null && (
                          ...
                          {dropOff === null && <div className="w-14 shrink-0" />}
                        </div>
                        {si < funnel.steps.length - 1 && (
                          <div className="ml-[38px] h-3 w-px bg-trackflow-bg-3" />
                        )}
                      </div>
                    );
                  })}
                ...
                <div className="px-6 py-3 border-t border-trackflow-bg-2 bg-trackflow-bg flex gap-6">
                  <div>
                    <span className="text-[10px] text-trackflow-text-3 uppercase tracking-wide">Total entered</span>
                    <span className="ml-2 font-mono text-[12px] text-trackflow-text">{funnel.steps[0]?.count.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-trackflow-text-3 uppercase tracking-wide">Completed</span>
                    <span className="ml-2 font-mono text-[12px] text-trackflow-text">{funnel.steps[funnel.steps.length - 1]?.count.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-trackflow-text-3 uppercase tracking-wide">Overall rate</span>
                    <span className="ml-2 font-mono text-[12px] text-trackflow-text">
                      {Math.round((funnel.steps[funnel.steps.length - 1]?.count / funnel.steps[0]?.count) * 100)}%
                    </span>
                  </div>
                </div>
              </div>
